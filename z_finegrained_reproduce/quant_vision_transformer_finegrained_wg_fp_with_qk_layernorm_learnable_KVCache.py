...
class Attention(nn.Module):
    def __init__(
        ...
        ###############Learnable KV_Cache#################
        self.learnable_cache = nn.Parameter(torch.randn(1, prefix_token_num, embed_dim))
    ...
    def forward(self, x, epoch, iteration, device_id, prefix_token_num):
        ...
        ###############learnable kv cache 추가#################
        prefix_cache = self.learnable_cache.expand(B, -1, -1)
        #cls token 과 image patch token 사이에 prefix_cache 삽입 
        #torch.cat 이용해서 prefix_cache  삽입하는 코드
        cls_token, image_patches = x[:, :1, :], x[:, 1:, :]
        x = torch.cat((cls_token, prefix_cache, image_patches), dim=1)  # (B, N + num_prefix_tokens, C)
        ######################################################
...
class Q_Block(nn.Module):
    def __init__(self, prefix_token_num, embed_dim, abits, wbits, w_gbits, a_gbits, block_num, dim, num_heads, mlp_ratio=4., 
...
class lowbit_VisionTransformer(VisionTransformer):
    def __init__(self, abits, wbits, w_gbits, a_gbits,
        ...
        self.blocks = CustomSequential(*[
            Q_Block(self.prefix_token_num, embed_dim, abits, wbits, w_gbits, a_gbits, block_num=i, dim=embed_dim,
